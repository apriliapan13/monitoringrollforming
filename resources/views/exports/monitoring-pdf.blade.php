...
th{background:#0c1a18;color:#fff;padding:8px 6px;text-align:left;font-size:10px;}
...
<h2>
Roll Forming - Cell 3<br>
Status : {{ request('status') ?: 'Semua' }} |
Bulan :
{{ request('month')
    ? \Carbon\Carbon::create()->month((int) request('month'))->translatedFormat('F')
    : 'Semua'
}}
...
<td>
<span class="badge {{ in_array($order->status,['ON TIME','FINISH']) ? 'badge-success' : ($order->status==='LATE' ? 'badge-danger':'badge-info') }}">
{{ $order->status }}
</span>
</td>
</tr>
...
<div class="footer">Sistem Monitoring Kapasitas Mesin RF </div>
